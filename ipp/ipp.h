...
/*
 * Defines parts of IPP protocol between the scheduler
 * and the printer. Based on RFC2911 and RFC2910
 */

#define STATCLASS_OK(x) ((x) >= 0x0000 && (x) <= 0x00ff)
#define STATCLASS_INFO(x) ((x) >= 0x100 && (x) <= 0x1ff)
#define STATCLASS_REDIR(x) ((x) >= 0x300 && (x) <= 0x3ff)
#define STATCLASS_CLIERR(x) ((x) >= 0x400 && (x) <= 0x4ff)
#define STATCLASS_SRVERR(x) ((x) >= 0x500 && (x) <= 0x5ff)

// Status codes.
#define STAT_OK 0x0000 // success
#define STAT_OK_ATTRIGN 0x0001 // OK; some attrs ignore
...
#define STAT_CLI_BADFMT 0x040a // unsuported doc format
...
#define STAT_SRV_UNAVAIL 0x0502 // service unavailabe
